...
class APIShuffle:
    ...
    def get_shuffle_the_deck_get(self):
        return self._request.get_request(f"{self.config['base_url']}/api/deck/new/shuffle/?deck_count={self.config['deck_count']}")

    def get_shuffle_the_deck_post(self):
        response = requests.post(f"{self.config['base_url']}/api/deck/new/shuffle/?deck_count={self.config['deck_count']}")
