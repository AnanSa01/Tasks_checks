...
class GetCompanyJobs:
    ...
    def api_post_get_company_jobs(self):
        response = self._request.post_request(f"{self.config["base_url"]}/company-jobs?{self.config["get_company_jobs_function"]}", self.config["header"],self.config["get_company_jobs_body"])
